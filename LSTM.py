...
class LSTM:
    ...
    def build(self, input_vgg: tf.placeholder, labels: tf.placeholder, n_hidden, init_type, learning_rate, opt_type,
              dropout=False, keep_prob=1):
        num_class = 51
        # input_vgg = tf.placeholder(tf.float32, shape=[n_frames, 1000, 1])  # input from vgg19
        ...
        cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=labels))
        if opt_type == 'adam':
