...
class LSTM:
    def __init__(self, input_vgg: tf.placeholder, labels: tf.placeholder, n_hidden, init_type, learning_rate, opt_type,
        ...
        self.pred, self.optimizer, self.loss = self.build(input_vgg, labels, n_hidden, init_type, learning_rate,
                                                          opt_type,
                                                          dropout, keep_prob)
    ...
    def build(self, input_vgg: tf.placeholder, labels: tf.placeholder, n_hidden, init_type, learning_rate, opt_type,
        ...
        cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=output, labels=labels))
        if opt_type == 'adam':
